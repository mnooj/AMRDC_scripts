...
def makeDotR(filename):
    ...
    header = data[0:3]
    month = int(filename[-8:-6])     # Extract month, year from filename
    ...
    # List of days in given month/year
    numDaysInMonth = monthrange(year, month)[1]
    dayList = [str(day).rjust(2, '0') for day in range(1, numDaysInMonth + 1)]

    # Create arbitrary list of hour/minute data points
    hourList = [n for n in range(24)]
    tenMinuteList = [n * 10 for n in range(6)]

    # Generate dictionary containing one key per hour/minute data point, i.e. a 'prototype' of daily data
    ...
    with open(filename+'.r', 'w') as f:
        f.writelines(header)
        for day in dayList:
            for observationNum, interval in enumerate(dailyIntervals.keys()):
                thisDatapoint = dataDict[day][interval]

                # .r = day + obs_num + temp + pressure + windspeed + wind direction + rel humidit
                # line formatting: ----0----0--000.0--000.0--000.0--000.0 ('-' are whitespace)
                # Note: Observation number is from enumerate() on line 71 (added 1 to index); 'day' has leading 0 stripped (if necessary)
                formattedLine = day.lstrip('0').rjust(5, ' ') + str(observationNum + 1).rjust(5, ' ') + thisDatapoint[0].rjust(7, ' ') + thisDatapoint[1].rjust(7, ' ') + thisDatapoint[4].rjust(7, ' ') + thisDatapoint[5].rjust(7, ' ') + thisDatapoint[10].rjust(7, ' ')
